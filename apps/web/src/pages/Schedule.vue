      ...
      <section v-else class="page-surface p-4 sm:p-5">
        ...
        <div v-else class="mt-5 grid gap-3">
          <article v-for="slot in todaySlots" :key="slot.id" class="rounded-2xl border p-4" :class="slot.overflowCount ? 'border-amber-200 bg-amber-50/50' : 'border-slate-200 bg-white'">
            <div class="flex min-w-0 items-center justify-between gap-3"><div class="min-w-0"><p class="text-lg font-bold text-slate-950">{{ formatOperationalTime(slot.startTime) }}</p><p class="mt-1 break-words text-xs font-semibold text-slate-500">{{ operationalSlotStateLabel(slot) }}<span v-if="slot.overflowCount"> · {{ slot.overflowCount }} overflow</span></p></div><span v-if="slot.overflowCount" class="shrink-0 rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.06em] text-amber-800">Overflow</span><span v-else-if="slot.bookedCount > 0" class="shrink-0 rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.06em] text-slate-600">Booked</span><span v-else class="shrink-0 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.06em] text-emerald-700">Available</span></div>
