<template>
  <main class="px-4 pb-28">
    ...
    <section class="page-surface p-4"><p class="field-label">AVAILABLE EXPERIMENTS</p><div class="mt-3 grid gap-3 sm:grid-cols-2"><RouterLink v-for="module in labModules" :key="module.id" :to="module.route" class="rounded-2xl border border-slate-200 p-4"><div class="flex items-center justify-between gap-3"><h2 class="text-base font-semibold text-slate-900">{{ label(module) }}</h2><span class="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-bold text-amber-700">LAB</span></div><p class="mt-2 text-sm text-slate-500">{{ description(module.id) }}</p></RouterLink></div><p v-if="labModules.length === 0" class="mt-3 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">No Labs are enabled for this identity.</p></section>
  </main>
</template>
...
function label(module: (typeof PLATFORM_MODULE_REGISTRY)[number]) { return module.id === "ROUTE_EXPERIMENT" && hunt.savedRouteCount > 0 ? module.label + " (" + hunt.savedRouteCount + ")" : module.label; }
function description(id: string) { return id === "LEAD_FINDER" ? "Location-based opportunity discovery for approved users." : id === "HOOD_NAVIGATOR" ? "Neighborhood exploration and territory clustering." : id === "INSTALLATION_SIGNALS" ? "Market signals for installation opportunity research." : "Review and manage the properties saved for your field route."; }
</script>
